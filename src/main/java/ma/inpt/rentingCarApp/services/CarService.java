...
public class CarService {
    ...
    // class constructor:
    public CarService(CarRepository carRepository, UserRepository userRepo) {
    ...
    public void removeReservation(Car car) {
        User reservedUser = car.getReservedByUser();
        for (int i = 0; i < reservedUser.getReversedCars().size(); i++) {
            if (reservedUser.getReversedCars().get(i).getCarId() == car.getCarId()) {
                reservedUser.getReversedCars().remove(i);
                break;
            }
        }
        userRepo.save(reservedUser);
